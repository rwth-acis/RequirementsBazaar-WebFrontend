<template>
    <div class="card" v-if="members">
        <div v-if="isModifyMembersAllowed">
            <Button :label="t('projectDetails-addMember')" icon="pi pi-plus" class="p-button-success p-my-3"
                @click="openNewMemberDialog" />
        </div>

        <DataTable :value="members" rowGroupMode="subheader" groupRowsBy="role" sortMode="single" sortField="role"
            :sortOrder="1" scrollable scrollHeight="800px">
            <Column field="userName" header="User">
                <template #body="slotProps">
                    <div class="user-field">
                        <div class="name">
                            <UserAvatar :imageUrl="slotProps.data.userProfileImage" :userName="slotProps.data.userName"
                                size="small" />
            ...
            <Column :exportable="false" header="Role">
                <template #body="slotProps">
                    <div class="role-field">
                        ...
                        <div class="controls">
                            <Button v-if="isModifyMembersAllowed" icon="pi pi-pencil"
                                class="p-button-rounded p-button-success p-mr-2"
                                @click="openEditMemberDialog(slotProps.data)" />
                            <Button v-if="isModifyMembersAllowed" icon="pi pi-trash"
                                class="p-button-rounded p-button-danger" @click="confirmRemoveMember(slotProps.data)" />
                        </div>
                    </div>
                </template>
            </Column>
    ...
    <Dialog v-model:visible="showAddMemberDialog" :style="{ width: '450px' }" :header="t('projectDetails-addMember')"
        :modal="true" class="p-fluid">
        <div class="p-field">
            <label for="name">{{ t('username') }}</label>
            <AutoComplete id="name" required="true" v-model="selectedUser" :suggestions="userSuggestions"
                @complete="searchUser($event)" field="userName" forceSelection
                :class="{ 'p-invalid': memberSubmitted && !userNameInput }"
                :placeholder="t('projectDetails-chooseUserName')" />
            <small class="p-error" v-if="memberSubmitted && !product.name">{{
                t('projectDetails-userNameIsRequired')
            }}</small>
        </div>

        <div class="p-field">
            <label for="inventoryStatus" class="p-mb-3">{{ t('role') }}</label>
            <Dropdown id="inventoryStatus" v-model="memberToAdd.role" :options="assignableRoles"
                :placeholder="t('projectDetails-selectRole')">
        ...
        <template #footer>
            <div class="footer">
                ...
                <Button :label="t('save')" icon="pi pi-check" @click="submitMember" />
            </div>
        </template>
    </Dialog>

    <Dialog v-model:visible="showEditMemberDialog" :style="{ width: '450px' }" header="Edit Member" :modal="true"
        class="p-fluid">
        <div class="p-field">
        ...
        <div class="p-field">
            <label for="inventoryStatus" class="p-mb-3">{{ t('role') }}</label>
            <Dropdown id="inventoryStatus" v-model="memberToEdit.role" :options="assignableRoles"
        ...
        <template #footer>
            <div class="footer">
                <ProgressBar mode="indeterminate" class="mb-3" v-if="inProgress" />
                <Button :label="t('cancel')" icon="pi pi-times" class="p-button-outlined p-ml-2 p-mr-2"
                    @click="showEditMemberDialog = false" />
                <Button :label="t('save')" icon="pi pi-check" @click="submitUpdatedMember" />
            </div>
        </template>
    </Dialog>
    ...
    <Dialog v-model:visible="showRemoveMemberDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
        ...
        <div class="confirmation-content">
            ...
            <span v-if="memberToRemove">{{ t('projectDetails-areYouSureYouWantToRemove') }}
                <b>{{ memberToRemove.userName }}</b> {{ t('projectDetails-fromTheProject') }}</span>
        </div>
        <template #footer>
            <div class="footer">
                <ProgressBar mode="indeterminate" class="mb-3" v-if="inProgress" />
                ...
                <Button :label="t('yes')" icon="pi pi-check" @click="removeMember(memberToRemove)" />
            </div>
        </template>
    </Dialog>
</template>
